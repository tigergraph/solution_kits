...
extern "C"  void mark_digits (const char* const iToken[], uint32_t iTokenLen[], uint32_t iTokenNum,
    char *const oToken, uint32_t& oTokenLen) {

    string inputString = string(iToken[0], iTokenLen[0]);
    string result = "";
    for (int i = 0; i < inputString.length(); i++) {
        string markedString = inputString;
        markedString[i] = '*';
        result += markedString + "#false|";
    }

    result += inputString + "#true";

    oTokenLen = result.size();
